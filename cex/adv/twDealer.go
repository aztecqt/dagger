...
import (
	...
	"aztecqt/dagger/util/indacators"
...
func (b *TWDealer) Init(
	...
	if b.dlPrice.Length() == 0 {
		priceRecords := b.trader.Market().HistoryPrice(t0, t1)
		for _, pr := range priceRecords {
			b.dlPrice.UpdateDecimal(pr.Time.UnixMilli(), pr.Price)
		}
	}
