...
func CCyCttypeToInstId(ccy, contractType string) string {
	switch contractType {
	case "usd_swap":
		instId := fmt.Sprintf("%s-USD-SWAP", strings.ToUpper(ccy))
		return instId
	case "usdt_swap":
		instId := fmt.Sprintf("%s-USDT-SWAP", strings.ToUpper(ccy))
		return instId
	default:
