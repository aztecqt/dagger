...
func (b *BalanceImpl) RecordTempRights(r decimal.Decimal, t time.Time) {
	...
	b.lastTempTime = t
	logger.LogInfo(b.ccy, "rights:%v, frozen:%v, temp:%v, tempDetail:%v", b.rights, b.frozen, b.temp.val, b.temp.String())
}
...
func (b *BalanceImpl) RefreshRights(rights, frozen decimal.Decimal, tm time.Time) {
	...
	logger.LogInfo(b.ccy, "rights:%v, frozen:%v, temp:%v, tempDetail:%v, accurate:%v", b.rights, b.frozen, b.temp.val, b.temp.String(), accurate)
}
...
func (p *PositionImpl) RecordTempLong(long decimal.Decimal, t time.Time) {
	...
	if t.UnixMilli() > p.updateTime.UnixMilli() {
		...
		logger.LogInfo(p.logPrefix(), "recording temp long, temp=%v, time=%d", long, t.UnixMilli())
		logger.LogInfo(p.logPrefix(), "long : size:%v, temp:%v, tempDetail:%s", p.longAmount, p.longTemp.val, p.longTemp.String())
		logger.LogInfo(p.logPrefix(), "short: size:%v, temp:%v, tempDetail:%s", p.shortAmount, p.shortTemp.val, p.shortTemp.String())
	}
}
...
func (p *PositionImpl) RecordTempShort(short decimal.Decimal, t time.Time) {
	...
	if t.UnixMilli() > p.updateTime.UnixMilli() {
		...
		logger.LogInfo(p.logPrefix(), "recording temp short, temp=%v, time=%d", short, t.UnixMilli())
		logger.LogInfo(p.logPrefix(), "long : size:%v, temp:%v, tempDetail:%v", p.longAmount, p.longTemp.val, p.longTemp.String())
		logger.LogInfo(p.logPrefix(), "short: size:%v, temp:%v, tempDetail:%v", p.shortAmount, p.shortTemp.val, p.shortTemp.String())
	}
}
...
func (p *PositionImpl) RefreshLong(long decimal.Decimal, avgPx decimal.Decimal, tm time.Time) {
	...
	p.longAvgPx = avgPx
	p.updateTime = tm
	logger.LogInfo(p.logPrefix(), "refreshing long pos, sz:%v, ts:%d", long, tm.UnixMilli())
	logger.LogInfo(p.logPrefix(), "long : size:%v, temp:%v, tempDetail:%v", p.longAmount, p.longTemp.val, p.longTemp.String())
	logger.LogInfo(p.logPrefix(), "short: size:%v, temp:%v, tempDetail:%v", p.shortAmount, p.shortTemp.val, p.shortTemp.String())
	logger.LogInfo(p.logPrefix(), "accurate:%v", p.longAmount.Sub(amoutOrign).Sub(tempOrign.Sub(p.longTemp.val)))
}
...
func (p *PositionImpl) RefreshShort(short decimal.Decimal, avgPx decimal.Decimal, tm time.Time) {
	...
	p.shortTotal = p.shortAmount.Add(p.shortTemp.val)
	p.updateTime = tm
	logger.LogInfo(p.logPrefix(), "refreshing short pos, sz:%v, ts:%d", short, tm.UnixMilli())
	logger.LogInfo(p.logPrefix(), "long : size:%v, temp:%v, tempDetail:%v", p.longAmount, p.longTemp.val, p.longTemp.String())
	logger.LogInfo(p.logPrefix(), "short: size:%v, temp:%v, tempDetail:%v", p.shortAmount, p.shortTemp.val, p.shortTemp.String())
	logger.LogInfo(p.logPrefix(), "accurate:%v", p.shortAmount.Sub(amoutOrign).Sub(tempOrign.Sub(p.shortTemp.val)))
}
