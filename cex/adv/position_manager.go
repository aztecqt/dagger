...
func (p *PositionManager) Init(trader common.FutureTrader, onDeal OnMakerOrderDeal, logPrefix string) {
	p.trader = trader
	p.onDeal = onDeal
	if len(logPrefix) == 0 {
	...
	p.tkOpen = new(Maker)
	p.tkClose = new(Maker)
	p.mkOpen.Init(trader, true, false, 0, 0, "mkOpen")
	p.mkClose.Init(trader, true, false, 0, 0, "mkClose")
...
func (p *PositionManager) Update() {
	...
	if p.targetDir != common.OrderDir_Sell && p.short().IsPositive() {
		...
		if p.long().Sub(p.targetSz).GreaterThanOrEqual(util.DecimalOne) && p.dealDir == DealDir_Close {
			...
			isOpen = false
		} else if p.targetSz.Sub(p.long()).GreaterThanOrEqual(util.DecimalOne) && p.dealDir == DealDir_Open {
			// 补开仓位
			dir = common.OrderDir_Buy
		...
		if p.short().Sub(p.targetSz).GreaterThanOrEqual(util.DecimalOne) && p.dealDir == DealDir_Close {
			...
			isOpen = false
		} else if p.targetSz.Sub(p.short()).GreaterThanOrEqual(util.DecimalOne) && p.dealDir == DealDir_Open {
			// 补开仓位
			dir = common.OrderDir_Sell
