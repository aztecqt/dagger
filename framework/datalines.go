...
func (d *DataLines) Init(names []string, dir string, maxLength int, intervalMs int64) {
	d.Lines = make([]*DataLine, len(names))
	for i := range d.Lines {
		d.Lines[i] = &DataLine{}
		d.Lines[i].Init(names[i], maxLength, intervalMs, 0).WithFileDirAndPath(dir, names[i]+".dl")
	}
}
