...
func InitInflux(name string, cfg influxdb.ConnConfig) *Influx {
	...
	if conn, err := client.NewHTTPClient(client.HTTPConfig{
		...
		inf.ifdbConn = conn
		logger.LogImportant(inf.logPrefix, "connected to influxdb %s", cfg.Addr)
	} else {
		logger.LogPanic(inf.logPrefix, "connected to influxdb %s failed, err=%s", cfg.Addr, err.Error())
	}
	inf.Start()
	influxIns = inf
	return inf
